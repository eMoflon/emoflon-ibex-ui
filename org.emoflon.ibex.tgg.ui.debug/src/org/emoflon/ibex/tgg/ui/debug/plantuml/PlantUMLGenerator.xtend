...
class PlantUMLGenerator {
	...
	def static String visualise(Iterable<Graph> graphs, IUserOptions userOptions, DataProvider dataProvider) {
		...
		val matchEdges = graphs.flatMap[g | g.edges].getMatchEdges(srcNodes.values.flatten + trgNodes.values.flatten,  userOptions)
	...
	private def static String visualizeNodeGroup(Map<Node,  String> nodesToIDs, VisualizationLabelOptions nodeLabelOptions) {
		'''
		together {
	...
	private def static String visualizeEdges(Iterable<Edge> edges, Map<Node,  String> nodesToIDs, VisualizationLabelOptions edgeLabelOptions) {
		'''
		«FOR edge : edges»
	...
	private def static String visualizeEdge(Edge e, Map<Node, String> nodesToIDs, VisualizationLabelOptions edgeLabelOptions) {
		val color = 
			if(e.type == EdgeType.MATCH)
				objectMappingColor
	...
	private def static boolean shallBeDisplayed(Edge e, IUserOptions userOptions) {
		if(e.type === EdgeType.CORR && !userOptions.displayCorrContextForMatches)
			return false;
		if(e.srcNode.domain === Domain.SRC && e.trgNode.domain === Domain.SRC && !userOptions.displaySrcContextForMatches)
			return false;
		if(e.srcNode.domain === Domain.TRG && e.trgNode.domain === Domain.TRG && !userOptions.displayTrgContextForMatches)
			return false;
	...
	private def static Iterable<Node> getVisibleNodes(Graph graph, Domain domain, IUserOptions userOptions, Iterable<Edge> corrEdges) {
		return graph.nodes.filter[n | n.domain === domain && (n.shallBeDisplayed(userOptions) || n.isContainedIn(corrEdges))]
	}
	
	private def static Iterable<Edge> getVisibleEdges(Graph graph, Iterable<Node> visibleNodes, IUserOptions userOptions) {
		graph.edges.filter[e | e.type === EdgeType.NORMAL && e.shallBeDisplayed(userOptions) && e.isContainedIn(visibleNodes)]
	}
	...
	private def static Iterable<Edge> getMatchEdges(Graph graph, Iterable<Node> visibleNodes, IUserOptions userOptions) {
		return graph.edges.getMatchEdges(visibleNodes, userOptions)
	}
	
	private def static Iterable<Edge> getMatchEdges(Iterable<Edge> allEdges, Iterable<Node> visibleNodes, IUserOptions userOptions) {
		return allEdges.filter[e | e.type === EdgeType.MATCH && e.shallBeDisplayed(userOptions) && (e.isContainedIn(visibleNodes))]
	}
