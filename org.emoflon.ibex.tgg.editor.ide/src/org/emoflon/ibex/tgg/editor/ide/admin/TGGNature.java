package org.emoflon.ibex.tgg.editor.ide.admin;
...
public class TGGNature implements IProjectNature {
	public static final String IBEX_TGG_NATURE_ID = "org.emoflon.ibex.tgg.editor.ide.nature";
	...
	private static Logger logger = Logger.getLogger(TGGNature.class);
	...
	public void configure() throws CoreException {
		PlatformUI.getWorkbench().getDisplay().syncExec(() -> {
			try {
				TGGNature.this.performSetUpRoutines();
				TGGNature.this.addIBexTGGBuilder();
			} catch (CoreException | IOException e) {
				LogUtils.error(TGGNature.logger, e);
			}
		});
	}
	...
	private void addIBexTGGBuilder() throws CoreException {
		...
		command.setBuilderName(TGGNature.IBEX_TGG_BUILDER_ID);
	...
	private void setUpAsJavaProject(final IProject project, final IProgressMonitor monitor) {
		...
		PlatformUI.getWorkbench().getDisplay().syncExec(() -> {
			...
			try {
				...
				TGGNature.logger.error("Exception during setup of Java project", e);
			}
		});
	}
	...
	private void setUpAsXtextProject() throws CoreException {
		WorkspaceHelper.addNature(this.project, TGGNature.XTEXT_NATURE_ID, new NullProgressMonitor());
	}

	private void setUpAsPluginProject() throws CoreException, IOException {
		WorkspaceHelper.addNature(this.project, TGGNature.PLUGIN_NATURE_ID, new NullProgressMonitor());
		this.setUpBuildProperties();
		this.setUpManifestFile();
		TGGNature.addContainerToBuildPath(this.project, "org.eclipse.pde.core.requiredPlugins");
	}
	...
	public static void addContainerToBuildPath(final IProject project, final String container) {
		TGGNature.addContainerToBuildPath(JavaCore.create(project), container);
	}
	...
	private static void addContainerToBuildPath(final IJavaProject iJavaProject, final String container) {
		try {
			...
			TGGNature.addContainerToBuildPath(classpathEntries, container);

			TGGNature.setBuildPath(iJavaProject, classpathEntries);
		} catch (JavaModelException e) {
			LogUtils.error(TGGNature.logger, e, "Unable to set classpath variable");
		}
	}
	...
	private static void setBuildPath(final IJavaProject javaProject, final Collection<IClasspathEntry> entries)
		...
		TGGNature.setBuildPath(javaProject, entries, new NullProgressMonitor());
	}

	private void setUpBuildProperties() throws CoreException {
		TGGNature.logger.debug("Adding build.properties");
	...
	private void setUpManifestFile() throws CoreException, IOException {
		TGGNature.logger.debug("Adding MANIFEST.MF");
		new ManifestFileUpdater().processManifest(this.project, manifest -> {
