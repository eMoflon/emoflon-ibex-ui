package org.emoflon.ibex.tgg.editor.ide.transformation;
...
public class EditorTGGtoFlattenedTGG {
	...
	/**
	 * Produces a flattened {@linkplain TripleGraphGrammarFile} from a given non-flattened
		...
		for (ObjectVariablePattern pattern : patterns) {
			...
			for (AttributeAssignment assign : pattern.getAttributeAssignments()) {
				if (!duplicateAssigns.contains(assign)) {
					duplicateAssigns.addAll(pattern.getAttributeAssignments().stream().filter(a -> !a.equals(assign) && identical(a, assign))
			...
			for (AttributeConstraint constr : pattern.getAttributeConstraints()) {
				if (!duplicateConstrs.contains(constr)) {
					duplicateConstrs.addAll(pattern.getAttributeConstraints().stream().filter(a -> !a.equals(constr) && identical(a, constr))
							.collect(Collectors.toList()));
				}
			}
	...
	private boolean identical(AttributeConstraint c, AttributeConstraint constr) {
		if (!c.getOp().equals(constr.getOp()))
			return false;
		if (!c.getAttribute().equals(constr.getAttribute()))
			return false;
	...
	private boolean identical(AttributeAssignment a, AttributeAssignment assign) {
		if (!a.getOp().equals(assign.getOp()))
			return false;
		if (!a.getAttribute().equals(assign.getAttribute()))
			return false;
	...
	private void cleanupCorrespondenceVariablePatternReferences(Rule rule) {
		rule.getCorrespondencePatterns().stream().forEach(c -> {
	...
	private void cleanupLinkVariablePatternReferences(Rule rule) {
		rule.getSourcePatterns().stream().flatMap(s -> s.getLinkVariablePatterns().stream())
				.forEach(l -> l.setTarget(this.sourcePatterns.get(l.getTarget().getName())));

		rule.getTargetPatterns().stream().flatMap(s -> s.getLinkVariablePatterns().stream())
	...
	private void cleanupAttributeExpressionReferences(Rule rule) {
		...
		rule.getAttrConditions().stream().flatMap(a -> a.getValues().stream()).filter(v -> v instanceof AttributeExpression)
				.forEach(a -> ((AttributeExpression) a).setObjectVar(objectPatterns.get(getObjectVarName(((AttributeExpression) a).getObjectVar()))));

		rule.getSourcePatterns().stream().flatMap(s -> s.getAttributeAssignments().stream()).map(a -> a.getValueExp())
				.filter(v -> v instanceof AttributeExpression)
				.forEach(a -> ((AttributeExpression) a).setObjectVar(objectPatterns.get(getObjectVarName(((AttributeExpression) a).getObjectVar()))));

		rule.getTargetPatterns().stream().flatMap(s -> s.getAttributeAssignments().stream()).map(a -> a.getValueExp())
				.filter(v -> v instanceof AttributeExpression)
				.forEach(a -> ((AttributeExpression) a).setObjectVar(objectPatterns.get(getObjectVarName(((AttributeExpression) a).getObjectVar()))));

		rule.getSourcePatterns().stream().flatMap(s -> s.getAttributeConstraints().stream()).map(a -> a.getValueExp())
				.filter(v -> v instanceof AttributeExpression)
				.forEach(a -> ((AttributeExpression) a).setObjectVar(objectPatterns.get(getObjectVarName(((AttributeExpression) a).getObjectVar()))));

		rule.getTargetPatterns().stream().flatMap(s -> s.getAttributeConstraints().stream()).map(a -> a.getValueExp())
				.filter(v -> v instanceof AttributeExpression)
				.forEach(a -> ((AttributeExpression) a).setObjectVar(objectPatterns.get(getObjectVarName(((AttributeExpression) a).getObjectVar()))));
	}
